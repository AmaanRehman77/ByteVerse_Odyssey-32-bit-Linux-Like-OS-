...
void terminal_init() {
    ...
    for (idx = 0; idx < 3; ++idx) {
        ...
        terminals[idx].attribute = 0x1F;

        terminals[idx].status     = 0;
        terminals[idx].r_flag     = 0;
        terminals[idx].stop     = 0;
        ...
        terminals[idx].history_index = 0;  // Corrected typo here
        terminals[idx].curr_history_ptr = 0;

        int k=0;
        int j=0;
        for (k = 0; k < HISTORY_SIZE; k++) {
            for (j = 0; j < BUFFER_LIMIT; j++) {
                terminals[idx].command_history[k][j] = '\0';
            }
        }
    }
...
void switch_terminal(uint8_t terminalNumber) {
    ...
    changeColor(terminals[next_terminal].attribute);
...
void scheduler(void) {
    ...
    if (terminals[sched_term].status == 0) {
        ...
        if(terminals[sched_term].stop == 1){
            terminals[sched_term].stop = 0;
            halt(0);
