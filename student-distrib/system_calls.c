...
int32_t execute (const uint8_t* command) {
    ...
    uint8_t * poop = "clear";
    if (strncmp((uint8_t*)cmd, poop, 32) == 0) {
        changeColor(0x1F);
        terminals[sched_term].attribute = 0x1F;
        sti();
        return 0;
    }

    uint8_t * pranav = "pranav";
    if (strncmp((uint8_t*)cmd, pranav, 32) == 0) {

        changeColor(0x1A);
        terminals[sched_term].attribute = 0x1A;
        printfk("Kernel Pranav\n");
        sti();
        return 0;
    }

    uint8_t * divyam = "divyam";
    if (strncmp((uint8_t*)cmd, divyam, 32) == 0) {

        changeColor(0x1B);
        terminals[sched_term].attribute = 0x1B;
        printfk("Kernel Divyam\n");
        sti();
        return 0;
    }

    uint8_t * amaan = "amaan";
    if (strncmp((uint8_t*)cmd, amaan, 32) == 0) {

        changeColor(0x1C);
        terminals[sched_term].attribute = 0x1C;
        printfk("Kernel Amaan\n");
        sti();
        return 0;
    }

    uint8_t * vikram = "vikram";
    if (strncmp((uint8_t*)cmd, vikram, 32) == 0) {

        changeColor(0x1D);
        terminals[sched_term].attribute = 0x1D;
        printfk("Kernel Vikram\n");
        sti();
        return 0;
    }
